...
void ComputeiLLNoisyOR(int *nOccCovs, int *nDetCovs, int *nSpecies, double *alphas, double *betas, double *beta0s, int* confusion, int *Yi, double *Xoi, double *Xdi, int *visit, int *Zs, int *nZs, double* iLL)
{
...
// compute log likelihood of site i
void ComputeiLLNoisyORNonIndependent(int *nOccCovs, int *nDetCovs, int *nSpecies, double *alphas, double *betas, double *beta0s, int* confusion, int *Yi, double *Xoi, double *Xdi, int *visit, int *Zs, int *nZs, double* iLL)
{
    // compute occ prob for all species
    double probOccs[1024];
    for (int s = 0; s < (*nSpecies); s++) {
    ...
    for (int k = 0; k < *nZs; k++) {
        ...
        double isL = 1;
        for (int s = 0; s < *nSpecies; s++) {
            if ( (s == 1 && Z[0] == 1) || (s == 3 && Z[2] == 1) ) {
                isL = isL * pow(probOccs[s]/2, Z[s]) * pow(1-probOccs[s]/2, 1-Z[s]);
            } else if (s == 4 && Z[2] == 1) {
                if (probOccs[s]*1.2 > 1) {
                    isL = isL * pow(1-EPSILON, Z[s]) * pow(EPSILON, 1-Z[s]);
                } else {
                    isL = isL * pow(probOccs[s]*1.2, Z[s]) * pow(1-probOccs[s]*1.2, 1-Z[s]);
                }
            } else {
                isL = isL * pow(probOccs[s], Z[s]) * pow(1-probOccs[s], 1-Z[s]);
            }
...
void ComputeEJLLNoisyOR(int *nSites, int *nVisits, int *nOccCovs, int *nDetCovs, int *nSpecies, double *alphas, double *betas, double *beta0s, double *probExpectedOccs, int *confusion, int *Y, double *Xo, double *Xd, int *visits, int *Zs, int *nZs, double *EJLL)
{
    *EJLL = 0;
...
void ComputeDerivsOfEJLLNoisyOR(int *nSites, int *nVisits, int *nOccCovs, int *nDetCovs, int *nSpecies, double *alphas, double *betas, double *beta0s, double *probExpectedOccs, int *confusion, int *Y, double *Xo, double *Xd, int *visits, int *Zs, int *nZs, int *fixedLeak, double *derivsOfEJLL)
{
...
// Predict Detection
void PredictDetNoisyOR(int *nSites, int *nVisits, int *nOccCovs, int *nDetCovs, int *nSpecies, double *alphas, double *betas, double *beta0s, int* confusion, double *Xo, double *Xd, int *visits, int *Zs, int *nZs, double *predictedDet)
{
    for (int i = 0; i < (*nSites); i++) {
        double *Xoi = Xo+i*(*nOccCovs);
...
// Predict Detection
void PredictDetNoisyORNonIndependent(int *nSites, int *nVisits, int *nOccCovs, int *nDetCovs, int *nSpecies, double *alphas, double *betas, double *beta0s, int* confusion, double *Xo, double *Xd, int *visits, int *Zs, int *nZs, double *predictedDet)
{
    for (int i = 0; i < (*nSites); i++) {
        double *Xoi = Xo+i*(*nOccCovs);
        ...
        for (int t = 0; t < visits[i]; t++) {
            ...
            for (int k = 0; k < (*nZs); k++) {
                ...
                for (int s = 0; s < (*nSpecies); s++) {
                    ...
                    if ((s == 1 && Z[0] == 1) || (s == 3 && Z[2] == 1)) {
                        probZOcc = probZOcc * pow(probOcc/2, Z[s]) * pow(1-probOcc/2, 1-Z[s]);
                    } else if (s == 4 && Z[2] == 1) {
                        if (probOcc*1.2 > 1) {
                            probZOcc = probZOcc * pow(1-EPSILON, Z[s]) * pow(EPSILON, 1-Z[s]);    
                        } else {
                            probZOcc = probZOcc * pow(probOcc*1.2, Z[s]) * pow(1-probOcc*1.2, 1-Z[s]);
                        }
                    } else {
                        probZOcc = probZOcc * pow(probOcc, Z[s]) * pow(1-probOcc, 1-Z[s]);
                    }
